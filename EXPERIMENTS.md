...
**Tentativa revertida (2026-08-01)**: um checkpoint novo (`classifier_2026-08-01_regressao.pt`) foi promovido e depois revertido no mesmo dia após confirmação de regressão -- ver seção "Regressão do checkpoint 2026-08-01" mais abaixo pra causa raiz (bug no fix de translação) e o achado do dry-run Kaggle (**71.9%**, 919/1278, 3 volumes). `classifier_best.pt` voltou a ser o de 2026-07-23 (86.78% continua sendo o número vigente). O run full-corpus no Kaggle desse checkpoint regredido segue rodando pra registrar o número oficial no histórico, mas não vai virar o ativo.
...
O checkpoint retreinado com os fixes acima (translação + morfologia + fontes) foi promovido e testado no Kaggle: recall caiu pra **71.9%** (919/1278, dry-run de 3 volumes/216 páginas) contra os 86.78% do checkpoint anterior -- confirmado também numa amostra local de 50 páginas (84.5% vs 88.1%) e um ETL9 baixo (4.89% top-1). Grande demais pra ser só ruído de amostra.

**Causa raiz identificada**: `apply_translate_and_crop` (`generate_crops.py`) é chamado em **100% das amostras** (não só nas ~70% em que a translação dispara de verdade), e o `crop_size` usava `max_dx`/`max_dy` fixos, independente de `disparou`. Resultado: TODA amostra do dataset -- inclusive as ~30% sem nenhuma translação -- passou a ser recortada mais apertada e reescalada de volta pro tamanho de saída. Com os valores de produção (`canvas_margin=0.15`, `CLF_TRANSLATE_MAX=0.10`), isso mudou o enquadramento do glifo de **76.9% do frame pra 96.2% do frame**, sistematicamente, no dataset inteiro -- uma mudança de escala muito maior e mais generalizada do que a redução de coocorrência morfologia+translação (que só afeta uma fração dos casos). Explica melhor a magnitude da queda do que a hipótese original.
