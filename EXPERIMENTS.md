...
**PENDENTE (2026-08-01)**: `classifier_best.pt` foi promovido pro checkpoint novo (ver seção do classificador acima), mas a validação full-corpus no Kaggle ainda não rodou. Amostra local pequena (50 páginas/252 pares) deu recall **84.5%**, pior que os 88.1% do checkpoint anterior na mesma amostra — sinal de possível regressão, não confirmado em escala. Não atualizar a leitura de "86.78% é o recall atual" até essa linha ser preenchida com o resultado do Kaggle.
